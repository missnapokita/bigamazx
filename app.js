(function () {
  ...
  function readPrefs() {
    try {
      ...
      if (providerById(prefs.providerId)) state.providerId = prefs.providerId;
  ...
  function renderProviders() {
    ...
    els.providerList.innerHTML = visible.map(function (provider) {
      return '<button class="provider-item' + (provider.id === state.providerId ? ' is-active' : '') + '" type="button" role="option" aria-selected="' + (provider.id === state.providerId ? 'true' : 'false') + '" data-provider="' + escapeHtml(provider.id) + '">' +
        '<span class="provider-logo provider-initial">' + escapeHtml(initials(provider.name)) + '</span>' +
        '<span class="provider-copy"><strong>' + escapeHtml(provider.name) + '</strong><small>' + escapeHtml(provider.id) + '</small></span>' +
        '<span class="provider-dot" aria-label="Direct API supported"></span>' +
      '</button>';
  ...
  function renderProviderHeader() {
    ...
    els.providerDocs.href = provider.docs;
  ...
  function buildEndpoint(provider, type, context) {
    ...
    if (provider.langParam && context.language) query[provider.langParam] = context.language;
  ...
  function apiFetch(provider, type, context) {
    ...
    return fetch(url, { method: 'GET', mode: 'cors', cache: 'no-store', referrerPolicy: 'no-referrer' }).then(function (response) {
  ...
  function normalizedDrama(raw, index, provider) {
    var id = nestedValueByKeys(raw, ['id', 'bookId', 'book_id', 'dramaId', 'drama_id', 'showId', 'videoId', 'video_id', 'cid', 'code', 'contentId', 'seriesId'], 2);
  ...
  function renderResults(append) {
    ...
    els.loadMoreWrap.hidden = !state.results.length;
  ...
  function openDrama(index) {
    ...
    detailRequest.then(function (payload) {
      ...
      if (!embeddedEpisodes.length && !provider.episodesPath) {
        ...
        renderEpisodes(direct ? [{ number: 1, title: 'Episode 1', episodeId: '1', fileId: '', raw: payload.original || payload }] : []);
      }
      return null;
    }).then(function () {
